...
void LaneAggregatingEvaluator::LoadModel() {
  ...
  AINFO << "(pengzi) prediction lane aggregating evaluator." ;

  torch::set_num_threads(1);
  torch_obstacle_encoding_ptr_ = torch::jit::load(
      FLAGS_torch_lane_aggregating_obstacle_encoding_file, device_);

  AINFO << "(pengzi)load online obatacle encoding model:" <<FLAGS_torch_lane_aggregating_obstacle_encoding_file <<". thread:"<< this_id <<" .";

  torch_lane_encoding_ptr_ = torch::jit::load(
      FLAGS_torch_lane_aggregating_lane_encoding_file, device_);

  AINFO << "(pengzi)load online lane encoding model:" << FLAGS_torch_lane_aggregating_lane_encoding_file <<". thread:"<< this_id <<" .";

  torch_prediction_layer_ptr_ = torch::jit::load(
      FLAGS_torch_lane_aggregating_prediction_layer_file, device_);

AINFO  << "(pengzi) load online lane aggregating prediction layer"<< FLAGS_torch_lane_aggregating_prediction_layer_file
    <<".Thread:" << this_id << ".";
