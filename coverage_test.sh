...
#add this to collect test coverage for each test file.
  Test_Target=`bazel query 'kind(cc_.*, tests(//modules/... union //cyber/...))'`
  for build_target in $Test_Target; do
    dir=$(get_filename $build_target) 
    mkdir $newdir/$dir

    bazel test $DEFINES -c dbg --config=coverage build_target
    COV_DIR=data/cov
    rm -rf $COV_DIR
